...
(function() {
  $(document).ready(function() {
    var _form, floatToString, handleSubmitForm;
    floatToString = Rich.Number.floatToString;
    _form = {
      $form: $('#exchange_form'),
    ...
    handleSubmitForm = (function(_this) {
      return function() {
        $.ajax('/exchange', {
          type: 'POST',
          dataType: 'json',
          ...
          success: function(data, text, jqXHR) {
            ...
            return _form.$inputResult.val(text);
          }
        });
        return false;
      };
    })(this);
    _form.$form.on('submit', function(event) {
      event.preventDefault();
      handleSubmitForm();
      return false;
    });
    _form.$selectCurrency.on('change', function() {
      return handleSubmitForm();
    });
    _form.$selectCurrencyDestination.on('change', function() {
      return handleSubmitForm();
    });
    _form.$inputQuantity.on('input', function() {
      if (parseInt(_form.$inputQuantity.val()) > 0) {
        return handleSubmitForm();
      } else {
        return _form.$inputResult.val('');
      }
    });
    $('.btn-invert-currency').on('click', function() {
      var origin;
...
(function() {
  ...
  _number = {
    delimiter: '.',
    separator: ',',
    floatToString: function(number, precision) {
      precision = typeof precision === 'undefined' ? 2 : precision;
      return jQuery.number(_number.round(number, precision), precision, _number.separator, _number.delimiter);
    },
